            ...
            foreach (Masraf masraf in Masraflar)
            {
                ...
                else
                {
                    ...
                    if (eklenecekMi)
                    {
                        ...
                        listViewItem.SubItems.Add(masraf.FisBilgisi.Tutar.ToString("C2")); // C : Currency : 2 de virgülden sonra iki digit anlamındadir.
        ...
        private void btnSil_Click(object sender, EventArgs e)
        {
            if (listView1.SelectedIndices != null && listView1.SelectedIndices.Count > 0)
            {
                ...
                if (masraf != null)
                {
                    if (masraf.Durumu == MasrafDurum.OnayBekliyor)
                    {
                        DialogResult dialogResult = MessageBox.Show($"{masraf.FisBilgisi.No} fiş numaralı masrafı silmek istediğinize emin misiniz?", "Masrafı Sil", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);


                        if (dialogResult == DialogResult.Yes)
                        {
                            Masraflar.RemoveAt(selectedIndex);
                            // Masraflar.Remove(masraf); üst satırdaki silme kodu ile aynı görevdedir.
